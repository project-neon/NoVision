...
void colorDetection(cv::Mat src, cv::Mat &mask, cv::Mat &hsv, cv::Mat &tgt, cv::Scalar colors[], int it){
  ...
  //hsv.release();
	...
	//cv::erode(mask, mask, cv::getStructuringElement(cv::MORPH_RECT,cv::Size(3, 3), cv::Point(0,0)));
	//cv::dilate(mask, mask, cv::getStructuringElement(cv::MORPH_RECT,cv::Size(5, 5), cv::Point(0,0)));
...
bool detectCircles(cv::Mat &src,cv::Mat &tgt, std::vector<cv::Vec3f> &circles,
	...
	cv::Mat cimg;
    cv::cvtColor(src, cimg, cv::COLOR_BGR2GRAY);
    HoughCircles(cimg, circles, cv::HOUGH_GRADIENT, 0.1, (int)(60 * k),
    ...
    for( size_t i = 0; i < circles.size(); i++ ) {
        ...
        cv::circle( cimg, cv::Point(c[0], c[1]), c[2], cv::Scalar(0,0,255), 3, cv::LINE_AA);
        cv::circle( cimg, cv::Point(c[0], c[1]), 2, cv::Scalar(0,255,0), 3, cv::LINE_AA);
    }
...
/*			//Debug
			roi_debug = roi.copy()
			cv2.putText(roi_debug, "{}".format(str(meanBigCircle[0])), (1,10), cv2.FONT_HERSHEY_SIMPLEX,
			 0.25, (0, 0, 255), 1)
			cv2.putText(roi_debug, "{}".format(str(meanBigCircle[1])), (1,20), cv2.FONT_HERSHEY_SIMPLEX,
			 0.25, (0, 0, 255), 1)
			cv2.putText(roi_debug, "{}".format(str(meanBigCircle[2])), (1,30), cv2.FONT_HERSHEY_SIMPLEX,
			 0.25, (0, 0, 255), 1)
			//plots.append(roi_debug)*/
